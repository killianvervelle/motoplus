...
const ShowHeroSlider = async () => {
  try {
    ...
    if (!products.length) {
      console.warn('[home] hero_slider empty or missing');
      return null;
    }
...
const ShowCollections = async () => {
  try {
    ...
    console.error('[home] collections failed', e);
    return null;
  }
};
  ...
  try {
    const { products = [] } =
      (await getCollectionProducts({
        collection: config.shopify.collections.featured_products,
        reverse: false,
      })) || {};
    if (!products.length) {
    ...
    console.error('[home] featured_products failed', e);
    return null;
  }
};
