...
const ProductLayouts = ({ categories, vendors, tags, maxPriceData, vendorsWithCounts, categoriesWithCounts }: any) => {
  ...
  useEffect(() => {
    const handleOutsideClick = (event: MouseEvent) => {
      ...
      if (!target.closest('.collapse-container-class') && !target.closest('.filter-button-container') && isExpanded) {
        setExpanded(false)
      }

      // set input editing state to false when clicking outside the input
      if (!target.closest('#searchInput') && isInputEditing && document.getElementById('searchInput')) {
